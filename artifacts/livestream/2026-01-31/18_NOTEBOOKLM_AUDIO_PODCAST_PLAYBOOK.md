...
- 핵심: 라이브 중에 만든 산출물/문서/공지/레퍼런스를 즉시 소스로 넣고, **에피소드로 변환**한 뒤, 방송 후 확산(바이럴 방/클립/요약)까지 끊김 없이 연결할 수 있습니다.
- 운영 주의: 외부 배포 전에는 오디오/요약의 수치·고유명사(가격, 일정, 링크)를 원문 소스와 1회 교차검증해 오인용을 줄이고, 확인 전 재공유를 막습니다.
...
### 3) ‘스토리보드/샷 목표’ 자동 생성
- “방금 만든 기능을 소개하는 30초 데모 영상을 만들 거야. timecode 기반(0–5s/5–12s/…)으로 스토리보드 표를 작성해줘.”

---

## C) 라이브 후(바이럴/재사용 파이프라인: 진짜 ‘와!’)

### 1) Audio Overviews를 ‘에피소드’로 뽑는다
- 운영 팁(지연 대비): 라이브 직후에는 먼저 **Brief(2분)**를 생성해 공유하고, Deep Dive는 후속 게시로 분리하면 운영 리스크를 줄일 수 있음. (업로드 전 고유명사/수치/링크만 1회 대조)
- 포맷 추천:
  - Deep Dive: “이번 주말 라이브에서 뭘 만들었는지 + 왜 중요한지”
  - Brief: “2분 요약(사람들이 공유하기 좋음)”
  - Debate: “노코드 에이전트 vs 전통 개발 — 언제 뭐가 이득인가”(바이럴)

### 2) 커스텀 프롬프트(복사/붙여넣기용 추천)
