...
추가 원칙(솔로 운영 안전장치): 방송 중에는 문서는 가능하면 **읽기 전용**으로 두고, 타임스탬프/운영 로그는 **별도 메모앱에만** 쌓기 → 종료 직후 템플릿에 1회로 옮기기(라이브 중 과다 수정 방지).
...
## 3-step 빠른 운영(최소 동선)
0) 급하면 **한 화면 요약만**: `05_ONE_SCREEN_CHEATSHEET.md`
1) 라이브 중 진행: `30_RUN_OF_SHOW_14-18.md`
2) 라이브 중 메모: **영상 재생 시간 `__:__`만** 계속 찍기(메모앱/문서 아무거나 OK). **첫 줄은 `00:00 오프닝`**으로 시작 → 종료 직후 `35_TIMESTAMP_TEMPLATE.md`에 한 번에 정리
