...
**시간 기준(헷갈림 방지)**
- **KST(시계)**: 실제 운영 스케줄(14:00 시작 등)
- **영상 재생 시간 `00:00` 기준**: `__:__`로 메모/챕터/고정댓글 타임스탬프

**3-step 빠른 운영(최소 동선)**
1) 라이브 중 진행: `30_RUN_OF_SHOW_14-18.md`
2) 라이브 중 메모: `__:__`(영상 재생 시간)만 계속 찍기 → 종료 직후 `35_TIMESTAMP_TEMPLATE.md`에 정리
...
- 링크/고정댓글/CTA 표: `20_YT_TITLE_DESC_PIN.md` *(고정댓글의 업데이트 시각 `__:__`는 KST(시계)로, 타임스탬프 `__:__`는 영상 재생 시간으로 분리해서 적기. 엔딩 멘트는 `70_CTA_SOFTSELL.md` 참고)*
...
- 고정댓글은 **링크 1개 먼저** 붙이고(광고 티 ↓), 나머지 정리는 방송 후 타임스탬프로 마무리.
