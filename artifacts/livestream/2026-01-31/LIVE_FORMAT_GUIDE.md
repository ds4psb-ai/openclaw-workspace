...
- 자동 로그(이번 루프 변경 요약): `ZZ_LAST_AUTOLOG.md` *(파일이 없거나 비어 있으면 이번 루프는 변경 사항 없음으로 간주하고, 수동 생성/수정 없이 읽기 전용 확인만 수행)*
...
- 멘션 전송 결정식: **라이브 당일(KST) AND 미전송 AND 멘션 완료 로그 없음 AND 라이브 시작 전**일 때만 1회 *(시작 전 판단은 YouTube Studio 상태가 `Scheduled(예정됨)`일 때만)*
