...
## 라이브에서 30초로 말할 요약(복붙)
- "OpenClaw (구 Moltbot)는 텔레그램 같은 메신저를 **AI 에이전트의 리모컨**으로 바꿔주는 게이트웨이입니다. 해외에서도 ‘폰에서 명령 → 서버에서 실행’ 패턴으로 개인 비서를 운영하는 사례가 늘고 있고, 오늘 우리가 시연하는 방식도 같은 철학입니다. **자동화는 데모를 넘어 운영(로그/스케줄/버전)까지 붙어야** 힘이 생깁니다."
...
- 운영/보안(기본값은 “거절”, 필요한 것만 허용):
  ...
  - 메신저 명령은 **allowlist + 승인(approval) + `requireMention`/prefix + `dry-run` 우선 적용** 가드레일로 작게 시작(권장)
