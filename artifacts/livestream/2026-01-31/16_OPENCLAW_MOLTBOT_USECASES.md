...
## 라이브에서 30초로 말할 요약(복붙)
- "OpenClaw(구 Moltbot)는 텔레그램 같은 메신저를 **AI 에이전트의 리모컨**으로 만드는 게이트웨이예요. 요즘 해외에서도 '내 폰에서 명령 → 내 서버에서 실행' 같은 패턴으로 개인 비서를 굴리는 사례가 늘고 있고, 오늘 우리가 하는 것도 같은 철학입니다: **자동화는 데모가 아니라 운영(로그/스케줄/버전)까지 붙여야** 힘이 생깁니다."
...
- 운영/보안:
  - 외부 공개 대시보드 금지(로컬/사설망)
  - 토큰/키는 절대 노출 금지(레포/스크린 공유 시 특히)
  - 메신저 명령은 **allowlist/승인(qa)** + **명령 prefix/requireMention** 같은 가드레일을 걸고 작게 시작
