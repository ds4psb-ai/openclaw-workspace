...
- 운영/보안(기본값은 “거절”, 필요한 것만 허용):
  ...
  - 메신저 명령은 **allowlist + 승인(approval) + `requireMention`/prefix + `dryRun` 우선 적용** 가드레일로 작게 시작하고, **1회 실행 원칙(한 번에 하나만 실행 → 결과 확인 → 다음 실행)**을 지킵니다. *(처음 1~2회는 읽기 전용 명령으로 리허설 후 진행 권장)*
