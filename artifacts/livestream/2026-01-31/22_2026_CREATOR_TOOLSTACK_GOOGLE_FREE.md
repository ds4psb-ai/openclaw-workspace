...
### (빠른 근거 3줄) Google 공식 발표 요약
- **세로(9:16) 네이티브 생성**을 지원해서 Shorts 같은 모바일 숏폼에 바로 쓸 수 있음(크롭/품질 손실 최소화).
- **레퍼런스 이미지 기반으로 캐릭터/배경/오브젝트 ‘일관성’**을 더 잘 유지하도록 개선됨(같은 주인공으로 여러 씬 구성).
- **1080p 및 4K 업스케일** 옵션을 제공하고, Gemini 앱/YouTube/Flow/API/Vertex AI/Google Vids 등에서 사용 경로를 안내함.

### 라이브 데모 체크리스트(측정 가능)
- [ ] 레퍼런스 이미지 2장으로 **같은 인물/같은 의상** 9:16 클립 2개 생성 → “일관성 차이”를 화면 비교
- [ ] 같은 프롬프트로 **배경만 바꾼 버전 2개** 생성 → “주인공 유지” 확인
- [ ] 최종 1개를 **1080p(또는 4K)로 업스케일**해 “편집용 결과물”로 저장
